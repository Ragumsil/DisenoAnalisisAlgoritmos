...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  8 00:30:14 2022
...
class grafo:
    ...
    def Prim(self):
        ...
        for i in self.vertices:
            ...
            for v in self.nodos[i].vecinos:
                if set[v] is False and distance[v] > \
                        self.nodos[vecino[0]].distancia:
                    distance[v] = self.nodos[i].distancia + vecino[1]
                    parent[v] = u
        ...
        for i in self.nodos:
            ...
            if parent[i] is not None:
                g.agregarArista(parent[i], i,self.nodos[i].distancia + vecino[1])
    ...
    def layout(self):
        ...
        lista.append(int(len(enlaces))
        for arista in enlaces:
            lista.append(arista[0][0])
            lista.append(arista[0][1])
                     
        nodos = lista[0]
        aristas = lista[1]
        l = graph(nodos, aristas, lista)
        k1 = 1.0
        c1 = 2
        k2 = 0.1
        c2 = 1
    ...
    def conversion(self):
        ...
        lines.append(int(len(enlaces))
        for arista in enlaces:
            lines.append(int(arista[0][0]))
            lines.append(int(arista[0][1]))
        print(lines)
